...
public class ForecastsState extends State {
    ...
    public void loadForecasts() {
		Loader loader = _getNetwork().getLoader(RoutesLoaderContainer.class);
		if (loader == null) {
			_getNetwork().loadForContainer(new RoutesLoaderContainer(), _controller);
		} else {
	...
	public void staticLoaded(Loader loader) {
		if (loader.getContainer().getClass() == RoutesLoaderContainer.class) {
			loadForecasts();
